...
file2 <- "./data/CH_Econ_Indicators.csv"
...
corstarsl(cbind(Asset_data, Indicators)[-3:-1])[6:14,1:5] #Pearson Corr
corstarsl(cbind(Asset_data, Indicators)[-3:-1],type="spearman")[6:14,1:5] #Spearman Corr

corstarsl(cbind(Asset_data, Indicators)['2008-03/'])[6:14,1:5] #Pearson Corr
corstarsl(cbind(Asset_data, Indicators)['2008-03/'],type="spearman")[6:14,1:5] #Spearman Corr

#3.Scatter Plot
plot(coredata(Asset_data[,"HSCEI.Index"]), coredata(Indicators[,"CPMINDX.Index"]), pch=19)
abline(v=0, h=1)
abline(h=-1)
